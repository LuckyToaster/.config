...
class Screen {
    ...
    public:
        int width, height;
        char* name;
        Screen(char* name, int width, int height);
        void drawline(float x1, float y1, float x2, float y2);
        //void drawShape(Shape2 shape); // must learn how to do matrix operations with opengl (manipulate shape2 objects to rotate and project them)
        void update();
        void clear();
...
Screen::Screen(char* name, int width, int height) {
    this->width = width;
    this->height = height;
...
/*void Screen::drawShape(Shape2 shape) {
    glEnableClientState(GL_VERTEX_ARRAY);
        glVertexPointer(3, GL_FLOAT, 0, shape.vertices.data());
        glDrawArrays(GL_LINES, 0, shape.vertices.size() / 3);
    glDisableClientState(GL_VERTEX_ARRAY);
}*/
