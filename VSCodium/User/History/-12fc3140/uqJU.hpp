...
vec3 glmProject(vec4 p, vec3 cam, vec3 orient, vec3 surface) {
    ...
    if (cam_is_zero && orientation_is_zero) 
        return vec3(p.x, p.y, 0.0f);

    vec4 cam_as_v4(cam, 0.0f);
    vec4 d;

    if (orient_is_zero) d = point - cam_as_v4;
    else d = getGlmXYZRotationMat(orient.x, orient.y, orient.z) * (point - cam_as_v4);

    return vec3((surface.z / d.z * d.x) + surface.x, (surface.z / d.z * d.y) + surface.y); 
}
