...
vec3 glmProject(vec4 p, vec3 cam, vec3 cam_orient, vec3 surface) {
    bool cam_is_zero = cam.x == 0.0f && cam.y == 0.0f && cam.z == 0.0f;
    bool cam_angle_is_zero = cam_orient.x == 0.0f && cam_orient.y == 0.0f && cam_orient.z == 0.0f;

    if (cam_is_zero && orientation_is_zero) return vec3(p.x, p.y, 0.0f);
