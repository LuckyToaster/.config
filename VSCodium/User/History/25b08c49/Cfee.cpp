...
void draw_info_box(int screenWidth, int screenHeight, Rectangle &player, Camera2D &camera) {
    ...
    DrawText(TextFormat("Player x: %02.0f", player.x), 20, 145, 20, BLACK);
    DrawText(TextFormat("Player y: %02.0f", player.y), 20, 170, 20, BLACK);
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            draw_info_box(screenWidth, screenHeight, player, camera);
