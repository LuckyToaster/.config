...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            DrawRectangle( 10, 10, 250, 123, Fade(SKYBLUE, 0.5f));
            DrawRectangleLines( 10, 10, 250, 113, BLUE);

            DrawText("Free 2d camera controls:", 20, 20, 15, BLACK);
            DrawText("- Right/Left to move Offset", 40, 40, 15, DARKGRAY);
            DrawText("- Mouse Wheel to Zoom in-out", 40, 60, 15, DARKGRAY);
            DrawText("- A / S to Rotate", 40, 80, 10, DARKGRAY);
            DrawText("- R to reset Zoom and Rotation", 40, 100, 15, DARKGRAY);
            DrawText(TextFormat("Player x: %02i", player.x), 20, 120, 15, DARKGRAY);
            DrawText(TextFormat("Player y: %02i", player.y), 20, 140, 15, DARKGRAY);
        EndDrawing();
    }
