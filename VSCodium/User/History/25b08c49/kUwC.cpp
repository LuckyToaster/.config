...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            DrawRectangle( 10, 10, 400, 175, Fade(SKYBLUE, 0.5f));
            DrawRectangleLines( 10, 10, 400, 175, BLUE);

            DrawText("Free 2d camera controls:", 20, 20, 20, BLACK);
            DrawText("- Right/Left to move Offset", 40, 40, 20, DARKGRAY);
            DrawText("- Mouse Wheel to Zoom in-out", 40, 60, 20, DARKGRAY);
            DrawText("- A / S to Rotate", 40, 80, 20, DARKGRAY);
            DrawText("- R to reset Zoom and Rotation", 40, 100, 20, DARKGRAY);
            DrawText(TextFormat("Player x: %02i", player.x), 20, 120, 20, BLACK);
            DrawText(TextFormat("Player y: %02i", player.y), 20, 140, 20, BLACK);
            DrawText(TextFormat("FPS: %02i", GetFPS()), 20, 160, 20, BLACK);
        EndDrawing();
    }
