...
bool Screen::handleInput(Camera& c) {
  // Handle window closing event
  if (glfwWindowShouldClose(window)) {
    return false; // Signal window close
  }

  // Process keyboard input
  if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS) {
    return false; // Close window on Escape press
  }

  float cameraSpeed = 5.0f; // Adjust camera movement speed if needed

  // Update camera position based on key presses
  if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) c.pos.z -= 9.0;
  if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) c.pos.z += 9.0;
  if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS) c.pos.x -= 9.0;
  if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS) c.pos.x += 9.0;
  if (glfwGetKey(window, GLFW_KEY_Q) == GLFW_PRESS) c.pos.y += 9.0;
  if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS) c.pos.y -= 9.0;
