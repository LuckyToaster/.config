...
class Shape2 {
    public:
        ...
        vector<GLfloat> getProjectedVerticesAsGLfloat();
        void projectVertices();
        static Shape2 getCube();
};


void Shape2 projectVertices(vec3 camera, vec3 orientation, vec3 surface) {
    for (vec4 v : vertices) 
        projectedVertices.push_back(project(v));
}

vector<GLfloat> Shape2::getProjectedVerticesAsGLfloat() {
    vector<GLfloat> res; 
    for (vec3 projected_vertices)
}
